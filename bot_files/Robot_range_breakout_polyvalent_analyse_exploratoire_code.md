...
# --- Filtre années ---
# [] = toutes les années (analyse complète)
# [2015, 2016, ...] = analyse partielle sur ces années uniquement
# Utile pour les gros fichiers (Gold 22 Go, etc.)
# ⚠ Si YEARS_FILTER est renseigné, les paramètres proposés ne sont
#   valables que sur la période sélectionnée.
YEARS_FILTER = [2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025]
...
def run_analysis():
    ...
    if YEARS_FILTER:
        log.info(f" Années analysées : {YEARS_FILTER}")
        log.info(f" ⚠  Analyse PARTIELLE — paramètres valables uniquement")
        log.info(f"    sur la période {min(YEARS_FILTER)}–{max(YEARS_FILTER)}")
    else:
        log.info(f" Années analysées : TOUTES")
    log.info("="*70)
    ...
    def flush_day(day_id, ts_list, ask_list, bid_list):
        ...
        if YEARS_FILTER and d.year not in YEARS_FILTER: return  # Filtre années
